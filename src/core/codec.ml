...
let string_padding_of_length length = 4 - (length mod 4)
...
module Decode = struct
  ...
  let decode_float32 input = Int32.float_of_bits (decode_int32 input)
  ...
  let rec decode_bundle input =
    ...
    in
    decode_packets input [] >>= fun packets -> Ok Osc_types.{ timetag; packets }

  and decode_packet input =
    match decode_string input with
    | "#bundle" -> decode_bundle input >>= fun bundle -> Ok (Osc_types.Bundle bundle)
    | address ->
      decode_arguments input
      >>= fun args -> Ok Osc_types.(Message { address; arguments = args })
  ;;
end
...
module Encode = struct
  ...
  let encode_float32 output f = encode_int32 output (Int32.bits_of_float f)
  ...
  let encode_arguments output args =
    ...
    List.iteri
      (fun index arg -> Bytes.set typetag_string (index + 1) (typetag_of_argument arg))
      args;
